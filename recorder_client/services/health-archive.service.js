...
class HealthArchiveService {
  ...
  async getBloodGlucoseRecords(memberId, options = {}) {
    try {
      ...
      // 数据验证
      this.validateBloodPressureData(completeData);

      // 调用API添加
      const result = await PatientAPI.addBloodPressureRecord(memberId, completeData);
      
      // 更新本地缓存
      this.addHealthMonitoringToCache(memberId, { 
        ...result.data, 
        monitorType: 'bloodPressure',
        value: completeData.bloodPressure.systolic
      });
      
      // 检查是否需要触发警报
      if (completeData.abnormal) {
        await this.triggerBloodPressureAlert(memberId, result.data);
      }
