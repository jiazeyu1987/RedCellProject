...
console.log('🎯 用户池管理增强功能 - 压力测试工具（修复版）');
...
class EnhancedUserGenerator {
  ...
  static async createUser(connection, userData) {
    ...
    return result.insertId; // 返回自增的ID
  }
  ...
  static async createPayments(connection, userId, paymentHistory) {
    ...
    for (const payment of paymentHistory) {
      ...
      await connection.execute(sql, [
        ...
        payment.paymentStatus, // 对应status字段
        payment.orderNo,
        payment.paymentTime // 对应pay_time字段
      ]);
    }
  }
